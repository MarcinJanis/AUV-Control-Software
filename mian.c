...
int16_t accellRaw[3];
float accellScaled[3];
float accellAngle[3];
int16_t gyroRaw[3];
float gyroScaled[3];
float KF_errorMatrix[4];
float orientationAngles[3];

// debugging variables:
HAL_StatusTypeDef mpuInitStatus,mpuCommStatus;
int temp1=0;
...
int main(void)
{
  ...
  osThreadDef(ReadInput, readInputFcn, osPriorityIdle, 0, 128);
  ReadInputHandle = osThreadCreate(osThread(ReadInput), NULL);

  /* definition and creation of ProcInputData */
  osThreadDef(ProcInputData, processInputDataFcn, osPriorityIdle, 0, 128);
...
void processInputDataFcn(void const * argument)
{
  ...
  for(;;)
  {
	  ...
	  angleX = accellAngle[0];
	  angleY = accellAngle[1];
	  angleZ = accellAngle[2];
